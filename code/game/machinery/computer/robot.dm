...
/obj/machinery/computer/robotics/ui_act(action, params)
	...
	switch(action)
		...
		if("magbot")
			...
			if((istype(S) && S.hack_software) || isAdminObserver(usr))
				var/mob/living/silicon/robot/R = locate(params["ref"]) in GLOB.silicon_mobs
				if(istype(R) && !R.emagged && (R.connected_ai == usr || isAdminObserver(usr)) && !R.scrambledcodes && can_control(usr, R))
