...
/obj/item/card/id/captains_spare/temporary/proc/wipe_id()
	...
	if(istype(loc,/obj/structure/fireaxecabinet/bridge/spare)) //if somebody is being naughty and putting the temporary spare in the cabinet
		...
		holder.spareid = null
		if(holder.atom_integrity > holder.integrity_failure) //we dont want to heal it by accident
			holder.take_damage(holder.atom_integrity - holder.integrity_failure, BURN) //we do a bit of trolling for being naughty
		else
