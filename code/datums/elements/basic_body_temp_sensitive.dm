...
/datum/element/basic_body_temp_sensitive/proc/on_life(datum/target, delta_time, times_fired)
	...
	var/mob/living/basic_mob = target
	var/gave_alert = FALSE

	if(basic_mob.bodytemperature < min_body_temp)
		basic_mob.adjusthealth(cold_damage * delta_time)
		switch(cold_damage)
			if(1 to 5)
				basic_mob.throw_alert(ALERT_TEMPERATURE, /obj/screen/alert/cold, 1)
			if(5 to 10)
				basic_mob.throw_alert(ALERT_TEMPERATURE, /obj/screen/alert/cold, 2)
			if(10 to INFINITY)
				basic_mob.throw_alert(ALERT_TEMPERATURE, /obj/screen/alert/cold, 3)
		gave_alert = TRUE

	else if(basic_mob.bodytemperature > max_body_temp)
		basic_mob.adjusthealth(heat_damage * delta_time)
		switch(heat_damage)
			if(1 to 5)
				basic_mob.throw_alert(ALERT_TEMPERATURE,/obj/screen/alert/hot, 1)
			if(5 to 10)
				basic_mob.throw_alert(ALERT_TEMPERATURE, /obj/screen/alert/hot, 2)
			if(10 to INFINITY)
				basic_mob.throw_alert(ALERT_TEMPERATURE, /obj/screen/alert/hot, 3)
		gave_alert = TRUE
