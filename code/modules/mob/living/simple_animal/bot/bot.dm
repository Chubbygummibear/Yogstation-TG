...
/mob/living/simple_animal/bot/Topic(href, href_list)
	...
	switch(href_list["operation"])
		if("patrol")
			if(!issilicon(usr) && !isAdminObserver(usr) && !(bot_core.allowed(usr) || !locked))
				return TRUE
		...
		if("hack")
			...
			if(emagged != 2)
				...
				bot_reset()
		if("ejectpai")
			if(paicard && (!locked || issilicon(usr) || isAdminObserver(usr)))
...
/mob/living/simple_animal/bot/proc/topic_denied(mob/user) //Access check proc for bot topics! Remember to place in a bot's individual Topic if desired.
	...
	if(emagged == 2) //An emagged bot cannot be controlled by humans, silicons can if one hacked it.
		if(!hacked) //Manually emagged by a human - access denied to all.
			return TRUE
		else if(!issilicon(user) && !isAdminObserver(user)) //Bot is hacked, so only silicons and admins are allowed access.
			return TRUE
	return FALSE

/mob/living/simple_animal/bot/proc/hack(mob/user)
	var/hack
	if(issilicon(user) || isAdminObserver(user)) //Allows silicons or admins to toggle the emag status of a bot.
...
/mob/living/simple_animal/bot/proc/showpai(mob/user)
	var/eject = ""
	if((!locked || issilicon(usr) || isAdminObserver(usr)))
