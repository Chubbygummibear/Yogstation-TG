	...
	if(is_servant_of_ratvar(user))
		if(uses_power)
			if(!get_clockwork_power(POWER_WALL_TOTAL))
				to_chat(user, span_warning("[src] requires <b>[display_energy(POWER_WALL_TOTAL)]</b> of power to produce brass sheets!"))
				return
			adjust_clockwork_power(-POWER_WALL_TOTAL)
		playsound(src, 'sound/items/deconstruct.ogg', 50, 1)
		new/obj/item/stack/tile/brass(user.loc, 5)
		to_chat(user, span_brass("You use [get_clockwork_power() ? "some":"all"] of [src]'s power to produce <b>5</b> brass sheets. It now has access to <b>[display_energy(get_clockwork_power())]</b> of power."))
	...
	if(!get_clockwork_power(fabrication_values["power_cost"]))
		if(get_clockwork_power() - fabrication_values["power_cost"] < 0)
			if(!silent)
				var/atom/A = fabrication_values["new_obj_type"]
				if(A)
					to_chat(user, span_warning("You need <b>[display_energy(fabrication_values["power_cost"])]</b> power to fabricate \a [initial(A.name)] from [target]!"))
		return FALSE
	return TRUE
