...
/datum/reagent/blob/cryogenic_poison/reaction_mob(mob/living/exposed_mob, method=TOUCH, reac_volume, show_message, touch_protection, mob/camera/blob/O)
	reac_volume = ..()
	if(exposed_mob.reagents)
		...
		exposed_mob.reagents.add_reagent(/datum/reagent/blob/cryogenic_poison, 0.6*reac_volume)
	exposed_mob.apply_damage(0.2*reac_volume, BURN, wound_bonus=CANT_WOUND)

/datum/reagent/blob/cryogenic_poison/on_mob_life(mob/living/carbon/exposed_mob)
	exposed_mob.adjustBruteLoss(0.3*REAGENTS_EFFECT_MULTIPLIER, FALSE)
	exposed_mob.adjustFireLoss(1*REAGENTS_EFFECT_MULTIPLIER, FALSE)
	exposed_mob.adjustToxLoss(1*REAGENTS_EFFECT_MULTIPLIER, FALSE)
	. = 1
	..()
